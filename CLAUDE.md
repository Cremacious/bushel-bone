...
- ⏳ **Prototype v0.2 phase (milestone #2)** — nearly done. Remaining: the dash-punctuation scrub of the existing prototype and event prose (#26), then a Vercel proof-of-concept for testers. Voice rule in force: no dash punctuation (D-037).
...
*Last updated: end of Session 5. Built and merged into the prototype, subagent-driven with a Vitest+jsdom harness (40 tests): the Foreman feature (D-036), the first-run tutorial (#20), and the imagery layer of location plates + speaker portraits (#21 to #23). No-dash voice rule locked (D-037). Next: the dash scrub (#26), then a Vercel proof-of-concept for testers.*

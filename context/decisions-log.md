...
### D-054. v0.4: the minute-to-minute is a beat-driven loop with roles, a tightened economy, and events.
**Context:** the daily loop (D-049) played set-and-forget and the economy had no teeth (playtest: infinite-coin job spam, free seed, a dead coin pile, trivial labor, drowning resources, no reason past Year 1). The third pass (spec `2026-07-30-v04-beat-driven-loop-design.md`), built and merged Session 18.
**Chosen:**
- **Beat-driven loop.** Time **auto-advances**; the player acts only at **beats** (harvest, a failing hand, a scarcity threat, an event, the season's close), not day by day. `RUN_DAYS`/`interrupts()` are the beat engine; the beat screen is compact (fixes the UI overload). Supersedes the *feel* of D-049 (the systems stay).
- **Crew standing-order roles** (Fields / Woodcutting / Foraging / Rest) replace per-day task assignment; a hand works its role in the background; labor bites (a hand can tire to failing/death), so Rest is a real tradeoff.
- **The player's time is a season pool** (~5 actions/season, the D-043 vision), spent at beats; the per-day budget is retired.
- **The tightening (exploits killed):** seed is a real coin sink (buy at Tolliver's to plant, no coin fallback); jobs are season-scarce (~2/season, each costing a season action); starting abundance is cut (coin 75, larder 55) with a Day-1 Year-1 note (20). Tuned against the sim so a **careless line loses the farm by ~Year 2** and a careful line survives on a knife-edge (the "early game can genuinely hurt you" lethality, chosen this session).

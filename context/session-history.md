...
## Session 19 — v0.4 Phase 4 (clarity, variety & the clone reveal), all merged to main

A long playtest-feedback session on the merged v0.4 beat loop. Chris delivered ~22 player notes ("moving in the right direction"); captured them in a triage doc (`docs/superpowers/specs/2026-07-30-v04-playtest-notes-triage.md`), designed solutions, wrote one spec (`...-v04-phase4-clarity-reward-clone-reveal-design.md`) and five phased plans, and executed all five subagent-driven (fresh implementer + spec/quality review per task, then browser-verified). All merged to `main`. Green throughout, ending at **289 tests / 44 files**.

- **4A — the mechanical vocabulary.** `fxTag` overhaul: human labels (`strainOne` → "Tiredness · a hand") and correct valence (tiredness/dread UP is now **red**, fixing the sick-hand "work them through it +16" showing green). A first-time tip teaches that Dread is hidden **by design** (D-027 intact). Recovery re-tuned (rest 8→14, care 5→10) so a rest day visibly steps a hand's condition, with the transition recorded. Verified live: `.ctag.bad` renders red, `.ctag.good` green.
- **4B — season pool & timing + the opening beat.** Relabeled "Your own time this season: N of 5 left" + purpose hint; a two-tap confirm before spending an action; a "your crew's orders take effect as the days run on" line. **The playtest "it's already day 4" fix (D-055):** SOW and winter's season-open no longer auto-run past day 1, so every season now opens on a **guaranteed day-1 beat** (survey, set the crew, optionally spend, then "Let the days run on"). Sim curve held.
- **4C — visual & info fixes.** Root-caused the highlight line-break/spacing bug to a **CSS class collision** (bare `.wx { display:flex }` and `.omen { border/bg/padding }` leaking into the `hl wx`/`hl omen` prose spans); fixed with a specificity-winning `.prose .hl` reset + scoping the masthead `.wx`. Restyled the resource line into an amber-when-short **status strip**; added crop grow-time to the planting chips ("ripens ~10d · food"); added a **Dusk crew snapshot** ("Reuben, worn.") surfacing how the crew fared.
- **4D — the NPC & job variety grammar.** The town stops being "pick the biggest coin." New engine: a seeded **haggle** roll (win/hold/sour) and **jobs open a card** instead of paying flat coin, with `applyFx` factored. 5 job cards and all 16 talks reworked across four kinds (**payload / question / haggle / moral**), +8 new talk cards, seeded deck **rotation** for replay variety. Tags are **suppressed on question scenes** so a riddle is a real question, not "pick the green one." Sim held.
- **4E — the scripted clone reveal.** A `cloneRevealed` flag + `scriptSeen`; the wagon is **masked** pre-reveal (no "clone hands" spoiler, hiring gated). A one-time **nudge** (`reuben_hands`) fires on Sow ("too much ground for two, ask in town"). Approaching the wagon plays **`vane_reveal`** (authored against narrative-bible canon: vessels grown from the Marrow's seam, the land counts cruelty), the first **moral framing** (kind: +regard/−dread; cold: −regard/+dread), then hiring opens with an honest label. Plus a content push of **4 new event cards** (crows, dry spell, buyer rumor, the eerie capped-well omen, kept closed). Verified the whole reveal live end to end.

**Deferred / minor follow-ups noted:** the town screen still uses the old pool label ("You have 5 of the season to spend here", the 4B relabel only hit the beat screen); the town job list shows a flat "+N coin" preview tag that under-sells the now-varying card payoff; HIRE is render-gated, not reducer-gated (fine, the sim resolves reveal-then-hire). New locked decision **D-055** (the guaranteed day-1 opening beat). All prior onboarding items (#32, #35 to #42) and milestone-2 items (#24, #48, a Vercel proof-of-concept) still open.

**Next:** playtest 4A to 4E, then the milestone-2 items (art-direction doc #24, a Vercel testers build), and the production port when the design is validated.
